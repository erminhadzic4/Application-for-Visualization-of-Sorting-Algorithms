...
{
    public partial class MaxHeapForma : Form
    {
        ...
        private void MaxHeapForma_Load(object sender, EventArgs e)
        {
            UcitajBoju();
        }
        ...
        public void w_IzvrsiWorkera(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                SortEngine engine = new MaxHeapSort(niz, g, panel1.Height);
                while (!engine.Sortirana() && (!w.CancellationPending))
                {
                    engine.Sljedeci();
                }
                engine.OcitajLog(lblPoredjeno, lblPristupljeno);
